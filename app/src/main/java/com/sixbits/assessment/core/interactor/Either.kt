...
sealed class Either<out L, out R> {
     ...
     * @see Left
     */
    val isError get() = this is Left<L>
     ...
     * @see Right
     */
    suspend fun foldAsync(fnL: suspend (L) -> Any, fnR: suspend (R) -> Any): Any =
        when (this) {
            is Left -> fnL(a)
            is Right -> fnR(b)
        }
...
suspend fun <L, R> Either<L, R>.onSuccessAsync(fn: suspend (success: R) -> Unit): Either<L, R> =
    this.apply { if (this is Either.Right) fn(b) }
